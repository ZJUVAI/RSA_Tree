...
const long long SATSIZE = 5059853300;
const int SATNODES = 20000;
...
class RTree {
   protected:
    ...
    INNTYPE query[INNDIMS];
    static INNTYPE SATPOOL[SATSIZE];
    static INNTYPE Data[DATASIZE][INNDIMS];
    vector<SAT<ELEMTYPE, INNTYPE, INNDIMS, NUMDIMS> > sat;
   ...
   public:
    ...
    // INNTYPE* allAsk(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]);
    
    INNTYPE* Ask(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]);
   ...
   protected:
    ...
    struct PartitionVars {
        ...
        PartitionVars(int MAXNODES) {
            m_partition = new int(MAXNODES);
            m_branchBuf = new Branch[MAXNODES];
            printf("hhh\n");
        }
        ~PartitionVars() {
            delete[] m_partition;
    ...
    void InsertRectNotModify(Node* a_root, int a_level, const ELEMTYPE* a_min, INNTYPE* insertData);
...
void RTREE_QUAL::InsertNotModify(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], INNTYPE* insertData) {
     InsertRectNotModify(m_root, 0, a_min, insertData);
}
...
void RTREE_QUAL::Debug() {
    for(int i = 0; i < NUMDIMS; ++i) printf("%d %d | ", rootInformation.m_min[i], rootInformation.m_max[i]); printf("\n");
...
RTREE_TEMPLATE
INNTYPE* RTREE_QUAL::Ask(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) {
    Rect a_rect;
...
// RTREE_TEMPLATE
// INNTYPE* RTREE_QUAL::allAsk(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS]) {
//     Rect a_rect;
...
//     INNTYPE* ans = new INNTYPE(INNDIMS);
//     for(int i = 0; i < INNDIMS; ++i) {
//         ans[i] = 0;
//     }
//     for(int i = 0; i < satTot; ++i) {
//         INNTYPE* tt = sat[i].calulate(a_rect.m_min, a_rect.m_max);
...
bool RTREE_QUAL::SaveRecAll(Node* a_node, RTFileStream& a_stream) {
    ...
    for (int index = 0; index < a_node->m_count; ++index) {
        ...
        // Node* curNode = curBranch->m_child;
        // a_stream.Write(curNode->m_level);
        // if(curNode->IsInternalNode()) {
        //     a_stream.Write(curNode->m_count);
        //     a_stream.WriteArray(curBranch->m_rect.m_min, NUMDIMS);
        //     a_stream.WriteArray(curBranch->m_rect.m_max, NUMDIMS);
        //  } else {
        //     a_stream.Write(curNode->satId);
        //     sat[curNode->satId].saveDomain(a_stream);
        // }
    ...
    // } else  // A leaf node
    // {
    //     // sat[a_node->satId].save(a_stream);
    // }
...
RTREE_TEMPLATE
void RTREE_QUAL::InsertRectNotModify(Node* a_node, int a_level, const ELEMTYPE* a_min, INNTYPE* insertData) {
    ASSERT(a_node);
    ASSERT(a_level >= 0 && a_level <= a_node->m_level);
    ...
    if (a_node->m_level > a_level) {
        ...
        // recursively insert this record into the picked branch
        InsertRectNotModify(a_node->m_branch[index].m_child, a_level, a_min, insertData);

        // a_node->m_branch[index].m_rect = CombineRect(&(a_node->m_branch[index].m_rect), a_min);
    } else if (a_node->m_level == a_level) {
        sat[a_node->satId].insertToTable(a_min, insertData);
    } else {
        // Should never occur
        ASSERT(0);
    }
...
void RTREE_QUAL::SplitNode(Node* a_node, const Branch* a_branch, Node** a_newNode, int MAXNODES, int MINNODES) {
    ...
    PartitionVars localVars(MAXNODES + 1);
    printf("hhh\n");
...
void RTREE_QUAL::ChooseAxisPartition(PartitionVars* a_parVars, int MAXNODES, int MINNODES) {
    ...
    firstChance = true;
    for(int i = MINNODES; i <= MAXNODES  - MINNODES + 1; ++i) {
...
void RTREE_QUAL::askDfs(Node* a_node, Rect* a_rect) {
    ...
    if (a_node->IsInternalNode()) {
        ...
        sat[a_node->satId].calulate(a_rect->m_min, a_rect->m_max, query);
    }
}
