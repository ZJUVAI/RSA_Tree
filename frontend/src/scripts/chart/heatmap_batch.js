...
import BatchDrawer from '../utility/BatchDrawer';
...
const COLORS_PRELOAD_NUM = 255;

const COLORS_PRELOAD_DEFAULT = Array.from(Array(COLORS_PRELOAD_NUM + 1)).map((_, i) => d3.rgb(COLOR_HEATMAP_DEFAULT(i / COLORS_PRELOAD_NUM)));

const COLORS_PRELOAD_ERROR = Array.from(Array(COLORS_PRELOAD_NUM + 1)).map((_, i) => d3.rgb(COLOR_HEATMAP_ERROR(i / COLORS_PRELOAD_NUM)));
...
function Heatmap(parameters) {
	this.parameters = Object.assign({
		...
		onSelect: (filter) => { 
			// console.log('Heatmap::onSelect', filter, '(default)'); 
		},
		onRefresh: () => { },
		refreshOptions: {},
	}, parameters);
...
Heatmap.prototype = {
	...
	init: function () {
		let parameters = this.parameters,
			...
			canvas = parameters.canvas, $canvas;
		...
		// }
		if (canvas == null) {
			let canvasId = `heatmap-canvas-${generateUUID({ sep: '_' })}`;
			$canvas = $('<canvas></canvas>').appendTo($(`#${map.getContainer()}`))
				.attr('id', canvasId);
			this.parameters.canvas = canvasId;
		} else {
			$canvas = $(`#${canvas}`);
		}
		...
		map.leaflet.on('zoomstart', () => { this.clear(); });
		map.leaflet.on('dragstart', () => { this.clear(); });
		...
		this.batchDrawer = new BatchDrawer($canvas[0], {
			maxLines: 2000 * 2000,
			// maxDots: 1000 * 1000,
			clearColor: {
				r: 0,
		...
		if (this.batchDrawer.error != null) {
			console.log(this.batchDrawer.error);
		}
	...
	render: function (data, boundingbox, resolution, options) {
		let parameters = this.parameters,
			map = parameters.map,
			// svg = parameters.svg;
			batchDrawer = this.batchDrawer;
		// drawHeatmap_d3(svg, data, map.toScreen_boundingbox(boundingbox), resolution, options);
		drawHeatmap_batchDrawer(batchDrawer, data, map.toScreen_boundingbox(boundingbox), resolution, options);
	},
...
export function drawHeatmap_batchDrawer(batchDrawer, data, boundingbox, resolution, options_) {
	let {
		options,
		x, y, width, height,
		color, opacity,
	...
	*/
	...
	// console.log(data);
	let scale = d3.scaleLog().domain([1, maxValue + 1]).range([0, COLORS_PRELOAD_NUM]);
	for (let d of data) {
		let x_v = x(d), y_v = y(d),
			width_v = width, height_v = height,
			// color_v = d3.rgb(color(d)),
			// color_v = COLORS_PRELOAD_DEFAULT[Math.floor(d.value / maxValue * (COLORS_PRELOAD_NUM))],
			color_v = options.useLog ? COLORS_PRELOAD_DEFAULT[Math.floor(scale(d.value + 1))] : COLORS_PRELOAD_DEFAULT[Math.floor(d.value / maxValue)],
			opacity_v = opacity(d);
		// 算颜色需要优化
		batchDrawer.addLine(x_v, y_v, x_v + width_v, y_v, height_v, color_v.r / 255, color_v.g / 255, color_v.b / 255, opacity_v);
	}
	batchDrawer.draw();
}
