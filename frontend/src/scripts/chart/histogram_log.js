...
Histogram.prototype = {
	...
	refresh: function (options_) {
		...
		let { domain_origins, domains } = fitGrid(targetDomain, targetBinWidth);

		console.log('Histogram_log::refresh', 'grid', domains, domain_origins);

		getValue({ domains }, (data) => {
			console.log('Histogram', data);
			// draw
			if (render) {
				this.render(data, domain_origins, domains.length, renderBackground);
			}
...
export function drawHistogram_d3(g, axesG, data, domain_origin, bins, width, height, options_) {
	...
	let xAxis = d3.axisBottom(xScale).ticks(binNum).tickFormat(d3.format('d')),
		yAxis = d3.axisLeft(yScale).ticks(5).tickFormat(d3.format('.2s'));
	axesG.selectAll("*").remove();
	if (options.drawXAxis) {
		let gXAxis = axesG.append('g')
			.attr('class', 'x-axis axes-rotate')
